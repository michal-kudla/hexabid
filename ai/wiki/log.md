...
## [2026-05-17] [FIX] Przywrócenie działania uwierzytelniania po błędach w authorization impl
- Komyty `bad74d1`, `99aedb0`, `cc4c024` (authorization backend impl) wprowadziły błędy w `LocalSecurityConfiguration`:
  - **`SessionCreationPolicy.STATELESS`** — psuje formLogin i /login/dev (wymagają sesji HTTP)
  - **`httpBasic(Customizer.withDefaults())`** — wysyła `WWW-Authenticate` header, wywołuje natywny dialog logowania w przeglądarce
  - **Brak `formLogin` i `oauth2Login`** — /login/dev nie działa
  - **Brak `permitAll` dla `/login/**`, `/logout`** — endpointy logowania niedostępne
  - **Brak `PasswordEncoder` bean** — Basic auth nie działa z `User.withDefaultPasswordEncoder()`
- Naprawiono `LocalAuthProviderDiscoverer` — zwracał pustą listę providerów zamiast "local" providera z `/login/dev`
- Naprawiono `LocalAuthProviderDescriptor` — loginUrl z `/login` na `/login/dev`
- Zaktualizowano E2E tests: marketplace.spec.ts, business-flow.spec.ts, pricing-create.spec.ts, auction-setup.spec.ts, qualification-profile.spec.ts, qualification-ux.spec.ts — dopasowano do aktualnego UI (nowy wizard setup, zmienione nagłówki)
- Dodano sekcję **NIE PSUJ UWIERZYTELNIANIA** w AGENTS.md z wyboldowanymi zakazami
- Wynik: 163 integration tests pass, 53 E2E tests pass, all unit tests pass
- Link: [[decisions/2026-05-05-dev-auth-e2e]]
- Tagi: #auth #fix #security #httpBasic #formLogin #session #passwordEncoder #e2e
